 ...
 *
 * Charge {@link PatientProfile} (agrégat Patient + ClinicalContext) depuis la
 * base H2 et publie des mesures temps réel sur MQTT.
 *
 * Les accès aux données physiologiques passent par les raccourcis de PatientProfile
 * (getBaseGlucose, getWeightKg…) qui délèguent à la sous-entité appropriée.
 */
@Service
...
public class MultiPatientSimulator {
    ...
    public void tick() {
        ...
        for (SimulatedDevice dev : activeDevices) {
            PatientProfile profile = patientRepo.findById(dev.getPatientId()).orElse(null);
            if (profile == null) continue;

            double value    = computeValue(dev, profile, ldt);
    ...
    private double computeValue(SimulatedDevice dev, PatientProfile p,
        ...
        double next = switch (dev.getDeviceType()) {
            ...
            case "SCALE"          -> evolveWeight(p);
            case "SPIROMETER"     -> evolveFev1(current);
            default               -> current;
        };
    ...
    private double evolveGlucose(double current, PatientProfile p,
        ...
        return Math.max(0.3, Math.min(5.5, current + drift + rng.nextGaussian() * 0.05));
    }

    private double circadianGlucoseDrift(int hour, double current, PatientProfile p) {
        double target = switch (hour) {
            case 7, 8   -> p.getBaseGlucose() + 1.0;
            case 12, 13 -> p.getBaseGlucose() + 1.1;
            case 19, 20 -> p.getBaseGlucose() + 0.9;
            case 2, 3   -> p.getBaseGlucose() - 0.15;
    ...
    private double evolveSpo2(double current, PatientProfile p,
        ...
        if ("DECOMPENSATION".equals(scenario))
            return Math.max(75.0, current - 0.3 + rng.nextGaussian() * 0.5);

        int hour = ldt.getHour();
        double target = (hour >= 22 || hour < 6) ? p.getBaseSpO2() - 1.5 : p.getBaseSpO2();
        return Math.max(75.0, Math.min(100.0,
                current + (target - current) * 0.15 + rng.nextGaussian() * 0.4));
    }

    private double evolveBP(double current, PatientProfile p, LocalDateTime ldt) {
        int hour = ldt.getHour();
        double target = (hour < 10 || hour > 18) ? p.getBaseSystolic() + 8 : p.getBaseSystolic();
        return current + (target - current) * 0.2 + rng.nextGaussian() * 4.0;
    }

    private double evolveWeight(PatientProfile p) {
        return p.getWeightKg() + rng.nextGaussian() * 0.1;
    }

    private double evolveFev1(double current) {
    ...
    private void publishMqtt(SimulatedDevice dev, PatientProfile profile,
        ...
        try {
            ...
            log.info("{} {} [{}] {} {} {} → {}",
                    icon, profile.fullName(), dev.getDeviceType(),
                    rec.getValue(), dev.getUnit(), rec.getSeverity(), topic);

        } catch (Exception ex) {
            log.error("Erreur publication MQTT {} : {}", dev.getSerial(), ex.getMessage());
        }
    }
